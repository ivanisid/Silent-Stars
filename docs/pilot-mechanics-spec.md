...
- **Відкладений бонд.** Без архетипу 8 XP не дають силу — лічильник скидається
  (`RESET_DEFERRED_XP`), і кожен скид рахується в `bond.deferredResets`.
...
#### Чого бракує для бондів

- **Переліку бондів немає взагалі.** Правила називають лише межі («від The Harlequin до
  The Seneschal») і один приклад major ideal. Тому архетип, перший major ideal, minor
  ideal і назви всіх сил вводяться текстом. Щоб зробити з них вибір зі списку, потрібні
  дані: архетипи, їхні перші major ideals, списки minor ideals і переліки bond powers.
- **Скільки сил дає вибір бонду — суперечність.** Правила кажуть і «перейшовши на Тір 2,
  можете одразу обрати бонд з двома bond powers», і, про відкладений бонд, «можете одразу
  додати відповідну кількість bond powers та +1 базовий». Для гравця з одним скидом це
  або 2, або 1+1 — збігається; для гравця з нулем скидів це 2 проти 1. Тому вибір бонду
  сил автоматично не нараховує: сили додаються вручну.
