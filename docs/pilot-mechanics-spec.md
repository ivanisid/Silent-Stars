...
### Що приховано з чарника

Панелі, прибрані з `PilotProfilePage` без видалення коду й даних. У кожному випадку
закоментовані імпорт і блок рендера, компонент лишається файлом (тому його екшени далі
покриває гард), стан пілотів у базі не чіпався, міграцій не потрібно.

| Панель | Коли | Що тягне за собою |
|---|---|---|
| `ProjectsPanel` | система проєктів | замінена трекером Get Creative |
| `DowntimePanel` (передмісійний «ДАУНТАЙМ») | кидки за столом | кнопка «НОВА МІСІЯ» переїхала у вкладку RESERVES |
| `HangarPanel` | до нових правил ангару | кап PR 200 дає лише «Ресурсний буфер» — купити його більше ніяк |
| `DowntimePanel` («ЧАС ПРОСТОЮ») | дії живуть у правилах і заявках | з ним пішли трекер Get Creative і опції Get rest |
| `ContactsPanel` | — | нічого: `state.contacts` більше ніхто не читає |

Секція бонду — окремий випадок: там прибрана саме розмітка (ідеали, архетип, сили),
а не компонент. Див. «Бонди: ідеали, XP і сили».
