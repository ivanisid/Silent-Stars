...
| Прогрес проекту | 1 | лишений як був |
...
- **Скільки секцій дає кидок Get Creative.** Правила описують це двічі по-різному: «на
  успіху 1 секція, на критичному успіху 2» і «1–9 → 1, 10–19 → 2, 20+ → 3». Кількість
  секцій лічильника вже однозначна — вона дорівнює рангу резерву.
- **Скільки проектів Get Creative можна вести.** Один варіант каже «не більше одного
  проекту одночасно», інший — що зберігати створені резерви можна в кількості, що
  дорівнює поточному тіру.
...
- **«Прогрес проекту» у новому переліку щотижневих дій відсутній**, але в додатку
  лишений: проєкти й `ADVANCE_PROJECT` існують у стані.
...
- **Ранг визначає тільки ціну.** Право купити резерв без downtime-дії залежить не від
  рангу, а від категорії й кількості: **без дії можна взяти рівно один мех-резерв**
  (`RESERVE_FREE_BUY_CATEGORY`, `RESERVE_FREE_BUY_MAX`), будь-якого рангу. Інші категорії
  — тактичні, для пілота, ресурсні — і кожна наступна покупка потребують дії. Безкоштовна
  покупка живе в `state.reserveFreeBuy` і відновлюється разом із місійними зарядами
  (`RESET_CHARGES` без `pool: 'weekly'`).
- **Саму downtime-дію додаток не списує.** `BUY_RESERVE` з `withAction: true` лише
  пропускає покупку й фіксує це в лозі: правила не кажуть, яка саме це дія.
...
- **Магазин отримав вкладки** (`ShopDrawer`): «РЕЗЕРВИ» за PR (з фільтрами за рангом і
  категорією та списком того, що вже на руках) і «ЗА МАНУ». Ширина шухляди 360 → 400.
...
- **Повний ремонт при підвищенні** застосовується до одного обраного меха. Пілот з одним
  ...
  без ремонту.
- **Платні перерозподіли:** повний перерозподіл усіх талантів і всіх ліцензій коштує по
  `REDISTRIBUTE_ALL_COST` = 100 мани кожен і додається до ціни підвищення.
...
- **Пороги мани виводяться формулою, а не задані списком** (`manaLevelCost` у `logic.js`).
  ...
  крок свого тіру, і крок береться за тіром рівня, **на який** іде підвищення — тому перехід
  між тірами вже дорожчає за новим тіром. Кроки: Тір 1 — 100, Тір 2 — 200, Тір 3 — 500.

  | Перехід | Тір, у який веде | Ціна | Накопичено від LL2 |
  |---|---|---|---|
  | LL2→LL3 | 1 | 1000 | 1000 |
  | LL3→LL4 | 1 | 1100 | 2100 |
  | LL4→LL5 | 1 | 1200 | 3300 |
  ...
  | LL6→LL7 | 2 | 1600 | 6300 |
  | LL7→LL8 | 2 | 1800 | 8100 |
  | LL8→LL9 | 2 | 2000 | 10 100 |
  | LL9→LL10 | 2 | 2200 | 12 300 |
  | LL10→LL11 | 3 | 2700 | 15 000 |
  | LL11→LL12 | 3 | 3200 | 18 200 |

  Повний шлях LL2 → LL12 коштує 18 200 мани.
...
- Ціна покращення ангару **«Ресурсний буфер»** — решта покращень Тіру 2 має ціну в
  мані + PR, у буфера її не вказано; `prices` лишений старий (1000).
