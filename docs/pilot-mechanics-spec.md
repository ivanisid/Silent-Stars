...
- **Магазин отримав вкладки** (`ShopDrawer`): «РЕМОНТ» за PR, «РЕЗЕРВИ» за PR (з фільтрами
  за рангом і категорією та списком того, що вже на руках) і «ЗА МАНУ». Ширина шухляди
  360 → 400. Вкладка ремонту показує `PR_SERVICES` — раніше цей список жив на панелі
  `PrPanel`; тепер панель лишає собі лічильник і кнопки переходу в магазин
  (`OPEN_SHOP_TAB`), щоб витратити PR можна було тільки в одному місці.
