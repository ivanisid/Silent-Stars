...
export default function ShopDrawer({ state, dispatch }) {
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, height: '100vh', display: 'flex', alignItems: 'stretch', zIndex: 40 }}>
      ...
      {state.shop.open && (
        <div style={{ width: 360, maxWidth: '85vw', height: '100vh', overflowY: 'auto', background: 'var(--panel)', borderLeft: '1px solid var(--header-border)', boxShadow: '-8px 0 30px var(--overlay)' }}>
          <div style={{ background: 'var(--header)', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10, position: 'sticky', top: 0, zIndex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-info)', marginLeft: 'auto' }}>БАЛАНС: {state.mana.balance} М</div>
          </div>
          <div style={{ padding: '8px 0 20px 0' }}>
            ...
            {SHOP_DATA.map((it) => (
              <div key={it.key} style={{ padding: '12px 20px', borderTop: '1px solid var(--rule)' }}>
                <div style={{ fontSize: 12, color: 'var(--text-soft)', lineHeight: 1.5, whiteSpace: 'pre-line' }}>{it.title}</div>
