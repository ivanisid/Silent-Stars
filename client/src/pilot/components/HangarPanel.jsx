  ...
  return (
    <div className="card">
      ...
      >
        ...
        <div style={{ fontSize: 11, color: '#9dc1e8', marginLeft: 'auto' }}>ПОКУПНІ БОНУСИ</div>
      ...
      {state.hangar.open && (
        <div>
          {HANGAR_DATA.map((item) => {
            ...
            return (
              <div key={item.key} style={{ borderBottom: '1px solid #16233490', padding: '18px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', gap: 5 }}>
                    {Array.from({ length: max }, (_, i) => (
                      <svg key={i} width="16" height="18" viewBox="0 0 20 23" style={{ display: 'block' }}>
                        <polygon points="10,1 19,6.5 19,16.5 10,22 1,16.5 1,6.5" fill={i < owned ? 'var(--accent)' : '#0c1826'} stroke="#3a6390" strokeWidth="1.5" />
                      </svg>
                    ))}
                  </div>
                  ...
                  <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}>
                    {done && (
                      <div style={{ fontSize: 11, color: 'var(--success)', letterSpacing: 1, border: '1px solid #2c5c3a', padding: '6px 12px' }}>ПРИДБАНО</div>
                    )}
                    {!done && (
                ...
                {item.desc && (
                  <div style={{ fontSize: 12, color: '#8ba5c3', lineHeight: 1.6, marginTop: 8, whiteSpace: 'pre-line' }}>{item.desc}</div>
                )}
                {item.levelTexts.map((text, i) => (
                  <div key={i} style={{ marginTop: 10, paddingLeft: 12, borderLeft: `2px solid ${i < owned ? 'var(--success)' : '#2a4a70'}` }}>
                    <div style={{ fontSize: 11, color: i < owned ? 'var(--success)' : 'var(--text-dim)', letterSpacing: 1 }}>
                      РІВЕНЬ {i + 1}{i < owned ? ' · ПРИДБАНО' : ''} · {item.prices[i]} М
                    </div>
                    <div style={{ fontSize: 12, color: '#8ba5c3', lineHeight: 1.6, marginTop: 3, whiteSpace: 'pre-line' }}>{text}</div>
                  </div>
                ))}
              </div>
            );
